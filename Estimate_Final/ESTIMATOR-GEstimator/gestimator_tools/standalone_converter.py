#!/usr/bin/env python3
"""
Standalone Excel to GEstimator Project Converter
Converts Excel project files to .eproj format without requiring GTK
"""
...
# GEstimator constants (copied to avoid GTK dependency)
...
# Write to log file
log_file = open("standalone_conversion.log", "w", encoding="utf-8")
...
def parse_excel_project(excel_path):
    ...
    log(f"  Column mapping: Code={code_col}, Desc={desc_col}, Unit={unit_col}, Rate={rate_col}, Qty={qty_col}, Amount={amount_col}")
...
def main():
    ...
    log("="*80)
    log("Excel to GEstimator Project Converter (Standalone)")
    log("="*80)
