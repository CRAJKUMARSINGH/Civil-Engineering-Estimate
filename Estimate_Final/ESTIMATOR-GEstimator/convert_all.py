#!/usr/bin/env python3
"""
Convert Excel files to GEstimator .eproj format and save to PROJECTS directory
Note: PDF files require manual OCR/data extraction and cannot be automatically converted
"""
...
# Constants
SOURCE_DIR = "Attached_Assets/PROJECTS"
TARGET_DIR = "PROJECTS"
...
def main():
    ...
    # Find PDF files (for information only)
    pdf_files = list(source_dir.glob("*.pdf"))
    if pdf_files:
        print(f"\nFound {len(pdf_files)} PDF files (cannot be automatically converted):")
        for f in pdf_files:
            print(f"  - {f.name}")
        print("  Note: PDF files require manual OCR and data extraction")
    
    # Convert each Excel file
    print(f"\n{'='*80}")
    print("Starting conversion of Excel files...")
    print(f"{'='*80}")
    ...
    print(f"PDF files found: {len(pdf_files)} (require manual processing)")
